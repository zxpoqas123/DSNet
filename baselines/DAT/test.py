...
def test(model, device, val_loader, criterion, logger, target_names):
    ...
    with torch.no_grad():
        for spec, emo_label, _ in tqdm(val_loader):
            ...
            embedding_ls.append(res['ebd'].cpu().numpy())                                                                                                                                                          
...
def test_epoch(model_root, fold, logger, model_name, max_len, feature):   
    ...
    if len(feature_ls)==1:
        model = Mymodel(feature_size=[feature_size_dic[feature] for feature in feature_ls], h_dims=128, emotion_cls=test_set.NumClasses, domain_cls=2, backbone_type=args.backbone_type)
