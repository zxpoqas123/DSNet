...
class EmoCLS(nn.Module):
    def __init__(self, emotion_cls=4, h_dims=256):
        ...
        self.fc1 = nn.Linear(h_dims, h_dims)
        self.activation = nn.PReLU()
        self.dropout = nn.Dropout(p=0.5)
        self.bn = nn.BatchNorm1d(h_dims)
    ...
    def forward(self, x):
        ...
        out = self.outlayer(self.fc2(self.bn(x)))
        return out

class DomCLS(nn.Module):
    def __init__(self, domain_cls=2, h_dims=256):
        super(DomCLS, self).__init__()
        self.fc1 = nn.Linear(h_dims, h_dims)
        self.activation = nn.PReLU()
        self.dropout = nn.Dropout(p=0.5)
        self.bn = nn.BatchNorm1d(h_dims)
        self.fc2 = nn.Linear(h_dims, domain_cls)
        self.outlayer = nn.LogSoftmax(dim=-1)
...
class Mymodel(nn.Module):
    def __init__(self, feature_size=[80], h_dims=256, emotion_cls=4, domain_cls=2, backbone_type='CNN6'):
        super(Mymodel, self).__init__()
        #self.bn1 = nn.BatchNorm2d(3)
        if backbone_type=='CNN6':
            self.backbone = CNN6(feature_size[0])
        self.att_block = AttBlock(512, h_dims, activation='sigmoid')
        self.EmotionClassifier = EmoCLS(emotion_cls, h_dims)
        self.DomainClassifier = DomCLS(domain_cls, h_dims)

    def forward(self, inputs, lambda_):
        x = self.backbone(inputs)
        embedding, att, _ = self.att_block(x)        
        reverse_ebd = GradientReversalFunction.apply(embedding, lambda_)        
        emo_pred = self.EmotionClassifier(embedding)
        domain_pred = self.DomainClassifier(reverse_ebd)
        res = {'ebd':embedding,'emo_pred':emo_pred,'domain_pred':domain_pred}
        return res