...
def test_epoch(model_root, fold, logger, model_name, max_len, feature):   
    ...
    if len(feature_ls)==1:
        model = Mymodel(feature_size=[feature_size_dic[feature] for feature in feature_ls], h_dims=128, emotion_cls=train_set.NumClasses, backbone_type=args.backbone_type)
...
def main(fold_list, fold_root, max_len, feature):
    ...
    for fold in fold_list:
        ...
        np.save(os.path.join(root,'embedding_train.npy'),res_train['embedding_all'])
        np.save(os.path.join(root,'label_train.npy'),res_train['true_all'])  
        np.save(os.path.join(root,'pred_train.npy'),res_train['pred_all'])
