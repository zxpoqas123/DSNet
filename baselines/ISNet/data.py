...
class Mydataset(Dataset):
    def __init__(self, dataset_type='IEMOCAP_4', mode='train', max_len=6, fold=0, num_neu=3, feature_ls=['FBank'], condition='impro'):
        data_all = pd.read_csv('/Storage/ASR/chenchengxin/NCMMSC2023/meta/{}.tsv'.format(dataset_type), sep='\t')
        ...
        if self.dataset_type=='MSP-Podcast':
            self.feature_root = ['/Storage/ASR/chenchengxin/NCMMSC2023/dataset/{}_{}_{}.pkl'.format(self.dataset_type, self.podcast[self.mode], feature_detail_dic[feature]) for feature in feature_ls]
        else:
            self.feature_root = ['/Storage/ASR/chenchengxin/NCMMSC2023/dataset/{}_{}.pkl'.format(self.dataset_type, feature_detail_dic[feature]) for feature in feature_ls]
        ...
        self.dic_neutral = self.get_neutral_dic(self.data_info, self.SpeakerNames, data_type=mode, num_neu=num_neu)
    ...
    def __getitem__(self, idx):
        ...
        speaker = speaker.astype('float').reshape(1)
        speaker = torch.Tensor(speaker).long().squeeze()

        neu_audios = self.dic_neutral[self.SpeakerNames[speaker]]
        neu_specs = [self.transform_ls[0](torch.tensor(self.feature_dic_ls[0][neu_audio])).float() for neu_audio in neu_audios]
        return spec, neu_specs, label
