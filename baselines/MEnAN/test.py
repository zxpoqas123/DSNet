...
from model import FocalLoss, Mymodel, EmoCLS
...
def test(ENC, EC, device, val_loader, criterion, logger, target_names):
    ...
    with torch.no_grad():
        for spec, emo_label, _ in tqdm(val_loader):
            ...
            embedding_ls.append(ebd.cpu().numpy())                                                                                                                                                          
...
def test_epoch(model_ENC_root, model_EC_root, fold, logger, model_name, max_len, feature):   
    ...
    test_set = Mydataset(dataset_type=dataset_type, mode='test', max_len=max_len, fold=fold, feature_ls=feature_ls, condition=condition)  
    
    if len(feature_ls)==1:
        ENC = Mymodel(feature_size=[feature_size_dic[feature] for feature in feature_ls], h_dims=128, backbone_type=args.backbone_type)
        bi = 2 if feature_ls[0] in ['WavLM','mfcc','spectrogram'] else 1      
        EC = EmoCLS(emotion_cls=test_set.NumClasses, h_dims=128*bi)

    ENC=torch.load(model_ENC_root)
    EC=torch.load(model_EC_root)
    ...
    res_dev = test(ENC, EC, device, dev_loader, criterion_test, logger, target_names)
    res_test = test(ENC, EC, device, test_loader, criterion_test, logger, target_names)
                                                                                                                                       
    return target_names,res_dev,res_test
...
def main(fold_list, fold_root, max_len, feature):
    ...
    for fold in fold_list:
        ...
        model_ENC_ls = list(i for i in os.listdir(root) if i.startswith('best_ENC_epoch'))
        epoch_ENC_ls = [int(i[:-3].split('_')[-1]) for i in model_ENC_ls]
        best_model_ENC = model_ENC_ls[np.argmax(epoch_ENC_ls)]
        model_ENC_root = os.path.join(root,best_model_ENC)

        model_EC_ls = list(i for i in os.listdir(root) if i.startswith('best_EC_epoch'))
        epoch_EC_ls = [int(i[:-3].split('_')[-1]) for i in model_EC_ls]
        best_model_EC = model_EC_ls[np.argmax(epoch_EC_ls)]
        model_EC_root = os.path.join(root,best_model_EC)

        target_names,res_dev,res_test = test_epoch(model_ENC_root, model_EC_root, fold, logger, best_model_EC[8:], max_len, feature)
        
        #test_epoch(model_root, condition, fold, logger, best_model)
        np.save(os.path.join(root,'embedding_dev.npy'),res_dev['embedding_all'])
        np.save(os.path.join(root,'label_dev.npy'),res_dev['true_all'])
        np.save(os.path.join(root,'embedding_test.npy'),res_test['embedding_all'])
        np.save(os.path.join(root,'label_test.npy'),res_test['true_all'])

        AR_ls_ls_dev.append(res_dev['AR_ls'])
        UAR_ls_dev.append(res_dev['UAR'])
        WAR_ls_dev.append(res_dev['WAR'])        
        F1_ls_ls_dev.append(res_dev['F1_ls'])
        UF1_ls_dev.append(res_dev['UF1'])
        WF1_ls_dev.append(res_dev['WF1'])
    ...
    logger.info('AR_ls_dev list: {}'.format(AR_ls_ls_dev))
    logger.info('UAR_dev list: {}'.format(UAR_ls_dev))
    logger.info('WAR_dev list: {}'.format(WAR_ls_dev))

    logger.info('AR_ls_dev avg: {}'.format(np.mean(AR_ls_ls_dev,0)))
    logger.info('UAR_dev avg: {}'.format(np.mean(UAR_ls_dev)))
    logger.info('WAR_dev avg: {}'.format(np.mean(WAR_ls_dev)))

    logger.info('F1_ls_dev list: {}'.format(F1_ls_ls_dev))
    logger.info('UF1_dev list: {}'.format(UF1_ls_dev))
    logger.info('WF1_dev list: {}'.format(WF1_ls_dev))

    logger.info('F1_ls_dev avg: {}'.format(np.mean(F1_ls_ls_dev,0)))
    logger.info('UF1_dev avg: {}'.format(np.mean(UF1_ls_dev)))
    logger.info('WF1_dev avg: {}'.format(np.mean(WF1_ls_dev)))
    ...
    with open('./test_all_seed_{}.txt'.format(seed),'a') as f:
        f.write('{} feature:{} max_len:{}\n'.format(args.dataset_type,feature,max_len))
        f.write('Class Names: {}\n'.format(target_names))

        f.write('AR_ls_dev avg: {}\n'.format(np.mean(AR_ls_ls_dev,0)))
        f.write('UAR_dev avg: {}\n'.format(np.mean(UAR_ls_dev)))
        f.write('WAR_dev avg: {}\n'.format(np.mean(WAR_ls_dev)))        
        f.write('F1_ls_dev avg: {}\n'.format(np.mean(F1_ls_ls_dev,0)))
        f.write('UF1_dev avg: {}\n'.format(np.mean(UF1_ls_dev)))
        f.write('WF1_dev avg: {}\n'.format(np.mean(WF1_ls_dev)))
    ...
    with open('./test_all_seed_{}_global.txt'.format(seed),'a') as f:
        f.write('{} feature:{} max_len:{}\n'.format(args.dataset_type,feature,max_len))
        f.write('Class Names: {}\n'.format(target_names))
