...
class SpkCLS(nn.Module):
    def __init__(self, speaker_cls=8, h_dims=256):
        super(SpkCLS, self).__init__()
        self.fc1 = nn.Linear(h_dims, h_dims)
        self.activation = nn.PReLU()
        self.dropout = nn.Dropout(p=0.5)
        self.bn = nn.BatchNorm1d(h_dims)
        self.fc2 = nn.Linear(h_dims, speaker_cls)
        self.outlayer = nn.LogSoftmax(dim=-1)
...
class Mymodel(nn.Module):
    def __init__(self, feature_size=[80], h_dims=256, backbone_type='CNN6'):
    ...
    def forward(self, inputs, lambda_):
        ...
        ebd, att, _ = self.att_block(x)        
        reverse_ebd = GradientReversalFunction.apply(ebd, lambda_)
        return ebd, reverse_ebd