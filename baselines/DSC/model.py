...
class AttBlock(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        if y!=None:
            norm_att = torch.softmax(torch.clamp(self.att(y), -10, 10), dim=-1)
        else:
...
class Mymodel(nn.Module):
    def __init__(self, feature_size=[80], h_dims=256, emotion_cls=4, backbone_type='CNN6'):
        ...
        if backbone_type=='CNN6':
            ...
            self.neu_backbone = CNN6(feature_size[0])
        self.att_block = AttBlock(512, h_dims, activation='sigmoid')
        #self.neu_att_block = AttBlock(512, h_dims, activation='sigmoid')
        self.softmax = nn.Softmax(dim=-1)
        self.EmotionClassifier = EmoCLS(emotion_cls, h_dims)

    def forward(self, inputs, neu_inputs):
        x = self.backbone(inputs)
        #embedding, att, _ = self.att_block(x)
        neu_x = self.neu_backbone(neu_inputs)
        #neu_embedding, neu_att, _ = self.neu_att_block(neu_x)
        #emo_ebd = embedding + self.fc(neu_embedding)
        #emo_ebd = embedding - neu_embedding
        #emo_ebd = embedding + self.softmax(neu_embedding)*embedding
        emo_ebd, att, _ = self.att_block(x, neu_x)
