...
def train(model, device, train_loader, criterion, optimizer, epoch, logger):
    ...
    for batch, data in tqdm(enumerate(train_loader)):
        spec, neu_spec, emo_label = data
        #spec, emo_label = [x.to(device) for x in spec], emo_label.to(device)
        spec, neu_spec, emo_label = spec.to(device), neu_spec.to(device), emo_label.to(device)                                                                                                       
        #spec, label_a, label_b, lam = mixup_data(spec, label, device, 0.2)
        optimizer.zero_grad()                                                                                                                                                                                      
                                                                                                                                                                                                                   
        #spec = aug(wav)
        emo_output,_ = model(spec, neu_spec)                                                                                                                                                                                       
...
def main():
    ...
    if len(feature_ls)==1:                                                                                                                                                                                                          
        model = Mymodel(feature_size=[feature_size_dic[feature] for feature in feature_ls], h_dims=128, emotion_cls=train_set.NumClasses, backbone_type=args.backbone_type).to(device)
