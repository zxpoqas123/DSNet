...
def pad_collate(batch):
    ...
    return xx_pack1, xx_pack2, y_emo
...
def test(model, device, val_loader, criterion, logger, target_names):
    ...
    with torch.no_grad():
        for spec, neu_spec, label in tqdm(val_loader):
            spec, neu_spec, label = spec.to(device), neu_spec.to(device), label.to(device)      
            ...
            #spec = aug(wav)
            output,embedding = model(spec, neu_spec)                                                                                                                                                                          
...
def main(fold_list, fold_root, max_len, feature):
    ...
    for fold in fold_list:
        ...
        target_names,res_dev,res_test = test_epoch(model_root, fold, logger, best_model, max_len, feature)
