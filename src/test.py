...
def test(model_dic, device, val_loader, cri_dic, logger, target_names):
    ...
    res = {'AR_ls':AR_ls,
        ...
        'true_all':true_all,
        'pred_all':pred_all,
        'spk_all':val_loader.dataset.speaker}                                                                                                                                                                                       
    return res
...
def test_epoch(model_root, fold, logger, model_name, max_len, feature):   
    ...
    res_train = test(model_dic, device, train_loader, cri_dic, logger, target_names)
    res_dev = test(model_dic, device, dev_loader, cri_dic, logger, target_names)
    res_test = test(model_dic, device, test_loader, cri_dic, logger, target_names)
                                                                                                                                       
    return target_names,res_train,res_dev,res_test
...
def main(fold_list, fold_root, max_len, feature):
    ...
    for fold in fold_list:
        ...
        target_names,res_train,res_dev,res_test = test_epoch(model_root, fold, logger, best_model, max_len, feature)
        
        #test_epoch(model_root, condition, fold, logger, best_model)
        np.save(os.path.join(root,'embedding_spk_train.npy'),res_train['embedding_spk_all'])
        np.save(os.path.join(root,'embedding_nonspk_train.npy'),res_train['embedding_nonspk_all'])
        np.save(os.path.join(root,'label_train.npy'),res_train['true_all'])        
        np.save(os.path.join(root,'spk_train.npy'),res_train['spk_all'])

        np.save(os.path.join(root,'embedding_spk_dev.npy'),res_dev['embedding_spk_all'])
        np.save(os.path.join(root,'embedding_nonspk_dev.npy'),res_dev['embedding_nonspk_all'])
        np.save(os.path.join(root,'label_dev.npy'),res_dev['true_all'])
        np.save(os.path.join(root,'pred_dev.npy'),res_dev['pred_all'])

        np.save(os.path.join(root,'embedding_spk_test.npy'),res_test['embedding_spk_all'])
        np.save(os.path.join(root,'embedding_nonspk_test.npy'),res_test['embedding_nonspk_all'])
        np.save(os.path.join(root,'label_test.npy'),res_test['true_all'])
        np.save(os.path.join(root,'pred_test.npy'),res_test['pred_all'])
    ...
    with open('./test_all_seed_{}.txt'.format(seed),'a') as f:
        f.write('{} feature:{} max_len:{} mode:{} \n'.format(args.dataset_type,feature,max_len,args.mode))
        f.write('Class Names: {}\n'.format(target_names))
...
if __name__ == '__main__':
    ...
    elif args.dataset_type.startswith('MSP-Podcast'):
