...
class CNN6(nn.Module):
    ...
    def forward(self, inputs):
        ...
        x = F.dropout(x, p=0.2, training=self.training)
        x = torch.mean(x, dim=3)  # x: (batch_size, 512, time_steps//16)
        '''
...
class Encoder(nn.Module):
    def __init__(self):
        super().__init__()
        self.layer1 = CNN6(80)
        self.layer2 = AttBlock(512, 256, activation='sigmoid')
        
    def forward(self, x):
        embedding, att, _ = self.layer2(self.layer1(x))
        return embedding

class Translator(nn.Module):
    def __init__(self):
        super().__init__()
        self.layer1 = AttentionTransLayer(256, 128)
        self.layer2 = AttentionTransLayer(128, 256)
