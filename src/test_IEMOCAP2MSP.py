...
def main(fold_list, fold_root, max_len, feature):
    ...
    for fold in fold_list:
        ...
        np.save(os.path.join(root,'embedding_spk_test_MSP.npy'),res_test['embedding_spk_all'])
        np.save(os.path.join(root,'embedding_nonspk_test_MSP.npy'),res_test['embedding_nonspk_all'])        
    ...
    with open('./test_all_seed_{}.txt'.format(seed),'a') as f:
        f.write('IEMOCAP2MSP-IMPROV feature:{} max_len:{} mode:{} \n'.format(feature,max_len,args.mode))
        f.write('Class Names: {}\n'.format(target_names))
