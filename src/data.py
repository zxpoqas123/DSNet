...
class Mydataset(Dataset):
    def __init__(self, dataset_type='IEMOCAP_4', mode='train', max_len=6, fold=0, num_neu=3, feature_ls=['FBank'], condition='impro'):
        ...
        for i in range(self.modalities):
            ...
            self.transform_ls.append(Augment(max_len=max_len, feature=feature_ls[i]))
        ...
        with open('/home/chenchengxin/spk_adaptiation_for_SER/dataset/speaker_vectors/wespeaker/{}_resnet34_256.pkl'.format(self.dataset_type), 'rb') as f:
            self.speaker_ebd_dic = pickle.load(f)
        self.dic_neutral = self.get_neutral_dic(self.data_info, self.SpeakerNames, data_type=mode, num_neu=num_neu, idx=0)
    ...
    def get_neutral_dic(self, df, speakers, data_type='val', num_neu=3, idx=0):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'val' or self.mode == 'test':
            neu_audio = self.dic_neutral[self.SpeakerNames[speaker]]
        else:
            neu_audio = random.choice(self.dic_neutral[self.SpeakerNames[speaker]])
        neu_spec = self.transform_ls[0](torch.tensor(self.feature_dic_ls[0][neu_audio])).float()

        return spec, neu_spec, label
